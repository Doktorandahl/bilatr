...
//
// The experimental model. phi (Dirichlet-multinomial concentration) is a
// per-dyad-period quantity, modeled as
// log(phi[d,t]) = log_phi0[d] + beta_logn * centered_log_n[d,t], where
// log_phi0[d] retains the per-dyad hierarchical structure (mu_log_phi,
// sigma_log_phi) and beta_logn is a single global slope capturing how
// overdispersion scales with observed count magnitude. Setting beta_logn's
// posterior near 0 recovers the per-dyad-constant-phi stable model.
//
// Both the per-dyad process_noise hierarchy and the log_phi0 hierarchy are
// sampled in non-centered form (log_process_noise_raw / log_phi0_raw ~
// std_normal(), with the constrained quantities built in transformed
// parameters), to keep mu_log_noise / sigma_log_noise / mu_log_phi /
// sigma_log_phi out of a funnel. The pre-0.3.0 centered version is kept
...
// prior alone does NOT recover the exact stable model bit-for-bit (phi is
// now time-varying via log_phi0 + beta_logn*0 at the mean), but is
// equivalent in spirit and nests it as beta_logn -> 0.
//
// Identification:
